...
<html>
<head>
	<title>Daftar Pertanyaan | IT Corner - Forum IT Terkini!</title>
    ...
    <div class="col-md-5 col-md-offset-2">
      ...
      <div class="col-md-12">
        ...
        <div class="panel-heading">
          <p class="panel-title" style="font-size:18px"><span class="glyphicon glyphicon-list"></span> Daftar Pertanyaan</p>
        </div>
          <div class="panel-body">
            ...
            <a class="list-group-item active clearfix">
              ...
              &nbsp;Pertanyaan Terbaru - <i>Semua kategori</i>
            ...
            echo '<span></a>';
            $query="select p.kunci, p.jumlah_jawaban, p.id_pertanyaan, p.judul_pertanyaan, p.kategori_pertanyaan, DATE_FORMAT(p.tanggal, '%d-%m-%Y - %H:%i') AS tanggal , u.nama from pertanyaan as p,user as u where p.username=u.username && p.status='1' order by p.id_pertanyaan desc LIMIT $startAt, $numContent";
            ...
            echo '<br><div class="list-group">
                ...
                <span class="glyphicon glyphicon-stats"></span>
                  &nbsp;Pertanyaan Terpopuler - <i>Semua kategori</i>
                </a>';
            $query="select p.kunci, p.id_pertanyaan, p.kategori_pertanyaan, p.judul_pertanyaan, DATE_FORMAT(p.tanggal, '%d-%m-%Y - %H:%i') AS tanggal_woy , u.nama, p.jumlah_jawaban, p.tanggal from pertanyaan as p,user as u where p.status='1' && p.username=u.username ORDER BY `p`.`jumlah_jawaban` DESC, `p`.`tanggal` DESC LIMIT 0 , 5";
            $rows=mysql_query($query);
            while($row=mysql_fetch_assoc($rows))
            {
              $kode = $row['id_pertanyaan'];
            ...
            <br><div class="list-group">
                ...
                <span class="glyphicon glyphicon-comment"></span>
                  &nbsp;Belum Terjawab - <i>Semua kategori</i>
                </a>';
            $query="select p.kunci, p.id_pertanyaan, p.kategori_pertanyaan, p.judul_pertanyaan, DATE_FORMAT(p.tanggal, '%d-%m-%Y - %H:%i') AS tanggal_woy , u.nama, p.jumlah_jawaban, p.tanggal from pertanyaan as p,user as u where p.jumlah_jawaban=0 && p.status='1' && p.username=u.username LIMIT 0 , 5";
            $rows=mysql_query($query);
            while($row=mysql_fetch_assoc($rows))
            {
              $kode = $row['id_pertanyaan'];
