        ...
        <div class="panel panel-default">
          ...
          if(isset($_SESSION['nama']))
          {
            if($_SESSION['nama'] == "Admin")
            {
              ...
              echo '<div class="alert alert-info">
                      ...
                      &nbsp;
                      <a href="list_pertanyaan.php"> <button type="button" class="btn btn-info">Kembali ke daftar pertanyaan</button></a>
                      &nbsp;
          ...
          if(mysql_query("select p.judul_pertanyaan, p.kategori_pertanyaan, DATE_FORMAT(p.tanggal, '%d-%m-%Y - %H:%i') AS tanggal , u.nama, p.isi_pertanyaan from pertanyaan as p,user as u where p.id_pertanyaan=".$kode." && p.username=u.username;"))
          {
            ...
            else
            {
              ...
              if($page > 5)
              {
                echo '<li><a href="isi_pertanyaan.php?id='.$kode.'&page=1" aria-label="Previous"><span aria-hidden="true">&larr;</span></a></li>';
              }

              if($page != 1)
              {
                $before = ($page - 1);
                echo '<li><a href="isi_pertanyaan.php?id='.$kode.'&page='.$before.'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
              }
              $i=1;
              ...
              {
                $after = ($page + 1);
                echo '<li><a href="isi_pertanyaan.php?id='.$kode.'&page='.$after.'" aria-label="Previous"><span aria-hidden="true">&raquo;</span></a></li>';
              }

              if(($totalpage - $page >= 4))
              {
                echo '<li><a href="isi_pertanyaan.php?id='.$kode.'&page='.$totalpage.'" aria-label="Previous"><span aria-hidden="true">&rarr;</span></a></li>';
              }
              echo '</ul>
                 </div></div></div>';
              //- end of pagination -//
              ...
              $result = mysql_query("SELECT j.id_jawaban, j.isi_jawaban, j.nama, DATE_FORMAT(j.tanggal, '%d-%m-%Y - %H:%i') AS tanggal FROM jawaban as j WHERE j.id_pertanyaan=".$kode." LIMIT $startAt, $numContent;");
                      ...
                      <div class="panel-body">
                        ...
                        if(isset($_SESSION['nama']))
                        {
                          if($_SESSION['nama'] == "Admin" || $_SESSION['nama'] == $row["nama"])
                          {
                            ...
                            echo '<a onclick="doConfirm(\''.$kodejawab.'\')"> <button type="button" class="btn btn-danger">Hapus jawaban</button></a></h5>';
                          }
                        }
                  echo '
                      </div>
          ...
          <?php
            ...
            elseif($_SESSION['nama'] != "Admin")
            {
              ...
              echo '<form method="post" action="tambah_konten.php?location='.$d.'&id='.$_GET['id'].'&aksi=jawab">
                      <p align="left"><textarea id="jawab" class="form-control" name="jawaban" rows="4" cols="136"></textarea></p>
                      <input class="btn btn-success" type="submit" value="Tambahkan"/>
                    </form>';
              echo '
                    </div>
             ...
             $loc = urlencode($_SERVER['REQUEST_URI']);
              echo '<script type="text/javascript">
                function doConfirm(id) 
                {
                  ...
                  {
                    window.location.href = "deletequery.php?id=" + id + "&location='.$loc.'&aksi=jawaban";
                  }
                }
              </script>';
          ?>
